...
[GitHub Releases](https://github.com/Kuroki-g/nidus/releases) から最新の `nidus-vX.Y.Z-<target>.tar.gz` をダウンロードして展開します。

```bash
# Linux x86_64 の例
tar xzf nidus-vX.Y.Z-x86_64-unknown-linux-musl.tar.gz
mv nidus ~/.local/bin/   # PATH が通っている場所に置く
```

#### cargo install（ソースビルド）
